...
- `install.sh` is the interactive orchestrator: it enforces non-root execution, can run in whiptail or `--tty` mode, auto-fixes APT sources (deb-src + missing non-free/non-free-firmware via an overlay file), removes any distro Hyprland packages before source builds, loads `hypr-tags.env` (refreshes `auto/latest`), then calls component installers via `execute_script`. Options (GTK themes, NVIDIA, dotfiles, SDDM, etc.) are either selected interactively or injected through `--preset <file>`. Trixie compatibility can be forced with `--build-trixie`/`--no-trixie`, and APT reinstalls with `--force-reinstall`.
