...
if [ -z "${glslang_DIR:-}" ] && ! find /usr/lib /usr/local/lib -maxdepth 4 -type f -name "glslangConfig.cmake" 2>/dev/null | grep -q .; then
    echo "${NOTE} glslang CMake config not found; attempting to install glslang-dev..." | tee -a "$LOG"
    install_package "glslang-dev" 2>&1 | tee -a "$LOG" || true
fi
