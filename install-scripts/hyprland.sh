...
EOF
    ...
    if [ -f "$EX_CONF" ] && [ -n "$DEFAULT_CFG_HDR" ]; then
        ...
        if command -v xxd >/dev/null 2>&1; then
            ...
            sed -ri 's|^\s*#embed\s+"\.{0,2}/\.{0,2}/example/hyprland\.conf"\s*$|#include "defaultConfig.bytes.inc"|g' "$DEFAULT_CFG_HDR" || true
        else
...
#include "defaultConfig.bytes.inc"
};
...
EOF
    ...
    if [ -f "$DEFAULT_CFG_HDR_LOWER" ] && [ ! -f "$DEFAULT_CFG_HDR_UPPER" ]; then
        cat >"$DEFAULT_CFG_SHIM" <<'EOF'
#pragma once
#include "defaultConfig.hpp"
EOF
    fi
    if [ -f "$DEFAULT_CFG_HDR_UPPER" ] && [ ! -f "$DEFAULT_CFG_HDR_LOWER" ]; then
        cat >"$DEFAULT_CFG_HDR_LOWER" <<'EOF'
#pragma once
#include "DefaultConfig.hpp"
EOF
...
if [ -z "${glslang_DIR:-}" ]; then
    ...
    if [ -n "$GLSLANG_CFG" ]; then
        export glslang_DIR="$(dirname "$GLSLANG_CFG")"
        echo "${NOTE} Found glslang CMake config at $GLSLANG_CFG" | tee -a "$LOG"
    else
        echo "${NOTE} glslang CMake config not found; attempting to install glslang-dev..." | tee -a "$LOG"
        if ! install_package "glslang-dev" "$LOG"; then
            echo "${NOTE} glslang-dev unavailable; attempting to install glslang-tools..." | tee -a "$LOG"
            install_package "glslang-tools" "$LOG" || true
        fi
    fi
fi
