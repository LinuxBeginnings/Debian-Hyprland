...
tag_default="auto"
if [ -z "${HYPRSHUTDOWN_TAG:-}" ]; then
  TAGS_FILE="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)/hypr-tags.env"
  [ -f "$TAGS_FILE" ] && source "$TAGS_FILE"
fi
TAG_SRC="${HYPRSHUTDOWN_TAG:-$tag_default}"
...
LOG="Install-Logs/install-$(date +%d-%H%M%S)_hyprshutdown.log"
MLOG="install-$(date +%d-%H%M%S)_hyprshutdown2.log"

SRC_DIR="$SRC_ROOT/hyprshutdown"
rm -rf "$SRC_DIR" 2>/dev/null || true
printf "${INFO} Installing ${YELLOW}hyprshutdown ${git_ref:-default-branch}${RESET} ...\n"
if git clone --recursive ${git_ref:+-b "$git_ref"} https://github.com/hyprwm/hyprshutdown.git "$SRC_DIR"; then
    ...
    if [ -f CMakeLists.txt ]; then
        cmake -S . -B "$BUILD_DIR" -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=/usr/local
    ...
    echo -e "${ERROR} Download failed for ${YELLOW}hyprshutdown${RESET}" 2>&1 | tee -a "$LOG"
fi
printf "\n%.0s" {1..1}